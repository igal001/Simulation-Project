...
void ICMPv6::handleMessage(cMessage *msg)
{
    ...
    else if(msg->getArrivalGate()->isName("f_RPL"))
    {
    	sendRPLControl(PK(msg));
    	return;
    }
}
...
void ICMPv6::processICMPv6Message(ICMPv6Message *icmpv6msg)
{
    ...
    else if(dynamic_cast<ICMPv6RPLControl *>(icmpv6msg))
    {
    	EV<<"ICMPv6 RPL Control Message Received";
    	processRPLcontrol((ICMPv6RPLControl *)icmpv6msg);
    }
    else
...
void ICMPv6::sendRPLControl(cPacket *msg)
{
	IPv6ControlInfo *ctrl = check_and_cast<IPv6ControlInfo*>(msg->removeControlInfo());
	ctrl->setProtocol(IP_PROT_IPv6_ICMP);
	ICMPv6RPLControl *request = new ICMPv6RPLControl(msg->getName());
	request->setType(ICMPv6_RPL_CONTROL);
	if(strcmp(msg->getName(),"RPL-DIS")==0)
	{
		request->setCode(DIS);
	}
	else if(strcmp(msg->getName(),"RPL-DIO")==0)
	{
		request->setCode(DIO);
	}
	else if(strcmp(msg->getName(),"RPL-DAO")==0)
	{
		request->setCode(DAO);
	}
	else if(strcmp(msg->getName(),"RPL-DAO_ACK")==0)
	{
		request->setCode(DAO_ACK);
	}
...
void ICMPv6::processRPLcontrol(ICMPv6RPLControl *icmpv6msg)
{
	IPv6ControlInfo *ctrl = check_and_cast<IPv6ControlInfo*>(icmpv6msg->removeControlInfo());
	cPacket *payload = icmpv6msg->decapsulate();
